    ...
    async def run(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        prompts = {
            'A': (
                ...
                f"The vocabulary and grammar must strictly match CEFR level {level} for language learners. "
                ...
                f"A2 level:\n"
                f"- 6–9 words per sentence.\n"
                f"- Very common everyday vocabulary only.\n"
                f"- No subordinate clauses.\n"
                f"- No adjectives that describe atmosphere (dark, silent, strange, etc.).\n\n"
                f"B1 level:\n"
                f"- 12–16 words per sentence.\n"
                f"- Exactly one subordinate clause using 'because', 'when', or 'that'.\n"
                f"- The sentence may show a simple reason or feeling.\n"
                f"- No complex imagery, no figurative language.\n"
                f"- Only everyday vocabulary used in B1 textbooks (work, travel, home, daily life).\n"
                f"- No literary style, no strong atmosphere words (twilight, shadows, unease, etc.).\n\n"
                f"B2 level:\n"
                f"- Up to two subordinate clauses.\n"
                f"- One linking word (however / although / yet).\n"
